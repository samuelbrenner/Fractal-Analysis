...
These wrinkles in the surface of a turbulent front can be viewed within the framework of fractal geometry, as was proposed by \cite{Mandelbrot1975} and further developed by \cite{Timmes1994}. Furthermore, \cite{Timmes1994} showed that the effective speed $v_{eff}$ of the flame front is given by
\begin{equation} 
v_{eff} = v_{cond} \left(\frac{l_{min}}{l_{max}}\right)^{2 - D}
\end{equation}
where $v_{cond}$ is the speed of the front through the material in the absence of turbulence, $l_{min}$ is the minimum length scale\footnote{I don't understand this well.} over which the turbulence takes place, $l_{max}$ is the maximum length scale over which the turbulence takes place, and $ D $ is the fractal dimension of the flame front.
